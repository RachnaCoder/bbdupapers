...
import React from "react";
import { LogIn, LogOut, Upload, FileText, Sun, Moon } from "lucide-react";
import { User } from "../types";
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
        <div className="flex justify-between h-16 sm:h-20 items-center">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            <div className="p-2 sm:p-2.5 bg-brand-600 rounded-xl shadow-lg shadow-brand-500/20">
              <FileText className="w-5 h-5 sm:w-6 sm:h-6 text-white" />
            </div>

            <span className="text-lg sm:text-2xl font-extrabold text-zinc-900 dark:text-white tracking-tight font-display">
              PAPERSTACK
            </span>
          </div>
          ...
          <div className="flex items-center gap-2 sm:gap-4">
            ...
            <button
              onClick={toggleDarkMode}
              className="p-2 sm:p-2.5 text-zinc-500 hover:text-brand-600 dark:text-zinc-400 dark:hover:text-brand-400 transition-all hover:bg-brand-50 dark:hover:bg-brand-900/20 rounded-xl"
            >
              {isDarkMode ? (
            ...
            {user ? (
              <div className="flex items-center gap-2 sm:gap-4">
                ...
                <button
                  onClick={onOpenUpload}
                  className="flex items-center gap-1 sm:gap-2 px-3 sm:px-5 py-2 sm:py-2.5 bg-brand-600 hover:bg-brand-700 text-white rounded-xl transition-all shadow-lg shadow-brand-500/20"
                >
                  <Upload className="w-5 h-5 sm:w-4.5 sm:h-4.5" />
                ...
                <div className="flex items-center gap-2 sm:gap-3 pl-2 sm:pl-4 border-l border-zinc-200 dark:border-zinc-800">
                  ...
                  <button
                    onClick={onLogout}
                    className="p-2 sm:p-2.5 text-zinc-500 hover:text-red-600 dark:text-zinc-400 dark:hover:text-red-400 transition-all hover:bg-red-50 dark:hover:bg-red-900/20 rounded-xl"
                  >
              ...
              <button
                onClick={onOpenAuth}
                className="flex items-center gap-1 sm:gap-2 px-4 sm:px-6 py-2 sm:py-2.5 bg-zinc-900 dark:bg-white text-white dark:text-zinc-900 rounded-xl hover:opacity-90 transition-all font-bold"
              >
                <LogIn className="w-5 h-5 sm:w-4.5 sm:h-4.5" />
